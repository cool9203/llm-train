...
def generate(
    ...
    **kwds,
    ...
    lora_request = None
    lora_modules = [lora_module["name"] for lora_module in __model.get("adapters", [])]
    if model_name and model_name in lora_modules:
        index = lora_modules.index(model_name)
        lora_request = LoRARequest(
            lora_name=model_name,
            lora_int_id=index,
            lora_path=__model.get("adapters")[index]["path"],
        )
    elif model_name and model_name in ["__base__", __model["name"]]:
        lora_request = None
    else:
...
def inference_table(
    ...
    **kwds,
    ...
    return [
        InferenceTableResponse(
            origin_content=generate_responses[i]["content"],
